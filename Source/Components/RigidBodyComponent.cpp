...
const float MAX_SPEED_X = 1250.0f;
const float MAX_SPEED_Y = 1250.0f;
const float GRAVITY = 2000.0f;

RigidBodyComponent::RigidBodyComponent(class Actor* owner, float mass, float friction, bool applyGravity, int updateOrder, float maxVelocity)
...
void RigidBodyComponent::Update(float deltaTime)
{
    ...
    if(mApplyGravity) {
        if (mCollidedWithSpikes == true)
            ApplyForce(Vector2::UnitY * 1500);
        else
            ApplyForce(Vector2::UnitY * GRAVITY);
    }

    if (mOwner->IsOnGround())
        mCollidedWithSpikes = false;
    ...
    auto velLength = mVelocity.Length();
    if(velLength > mMaxVelocity){
        mVelocity.Normalize();
